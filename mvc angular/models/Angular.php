...
namespace models;
class Angular extends \app\Model{
    ...
    */
    ...
    public function categorie($categories_id){//trier sur page epoque ou evenement
        $stmt = $this->_connexion->stmt_init();
        $stmt->prepare("select * from ".$this->table." where `categories_id`= ?");
        $stmt->bind_param("i", $categories_id);
        $stmt->execute();
        $stmt_result = $stmt->get_result();
        $result = $stmt_result->fetch_all(MYSQLI_ASSOC);
    ...
    public function accueil($test){
        $stmt = $this->_connexion->stmt_init();
        $stmt->prepare("select image, alt, css, categories_id, titre, contenu, slug from ".$this->table." where `accueil`= ?");
